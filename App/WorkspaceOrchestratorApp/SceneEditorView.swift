...
struct SceneEditorView: View {
    ...
    private func appendHealth(_ check: HealthCheck, to selection: ActionSelection) { updateConfiguration(selection) { $0.healthChecks.append(applyingHealthDefaults(to: check)) } }
    private func applyingHealthDefaults(to check: HealthCheck) -> HealthCheck {
        let interval = executionDefaults.healthCheckIntervalSeconds, attempts = executionDefaults.healthCheckMaximumAttempts
        switch check {
        case .http(var value): value.intervalSeconds = interval; value.maximumAttempts = attempts; return .http(value)
        case .tcp(var value): value.intervalSeconds = interval; value.maximumAttempts = attempts; return .tcp(value)
        case .process(var value): value.intervalSeconds = interval; value.maximumAttempts = attempts; return .process(value)
        case .application(var value): value.intervalSeconds = interval; value.maximumAttempts = attempts; return .application(value)
        case .file(var value): value.intervalSeconds = interval; value.maximumAttempts = attempts; return .file(value)
        case .docker(var value): value.intervalSeconds = interval; value.maximumAttempts = attempts; return .docker(value)
        }
    }
    ...
    @ViewBuilder private func payloadEditor(_ selection: ActionSelection) -> some View {
        if let action = action(for: selection) {
            ...
            case .managedProcess(let value): TextField("Absolute executable", text: payload(selection, value.executable) { var copy = value; copy.executable = $0; return .managedProcess(copy) }); StructuredArgumentEditor(executable: value.executable, arguments: argumentsBinding(selection)); TextField("Working directory", text: payload(selection, value.workingDirectory ?? "") { var copy = value; copy.workingDirectory = $0.isEmpty ? nil : $0; return .managedProcess(copy) }); TextField("Single-instance key", text: payload(selection, value.singleInstanceKey) { var copy = value; copy.singleInstanceKey = $0; return .managedProcess(copy) }); Picker("Restart policy", selection: Binding(get: { value.restartPolicy }, set: { var copy = value; copy.restartPolicy = $0; replace(selection, with: .managedProcess(copy)) })) { Text("Never").tag(RestartPolicy.never); Text("On failure").tag(RestartPolicy.onFailure) }; TextField("Graceful interrupt seconds", value: Binding(get: { value.gracefulStopSeconds }, set: { var copy = value; copy.gracefulStopSeconds = $0; replace(selection, with: .managedProcess(copy)) }), format: .number); TextField("Terminate before force-kill seconds", value: Binding(get: { value.forcedStopSeconds }, set: { var copy = value; copy.forcedStopSeconds = $0; replace(selection, with: .managedProcess(copy)) }), format: .number); Text("Environment entries are stored as typed plain/inherited/Keychain-reference values. Existing entries are preserved; use import JSON review for advanced environment editing.").font(.caption).foregroundStyle(.secondary)
