...
public struct ManagedProcessAction: Codable, Equatable, Sendable {
    ...
    public var gracefulStopSeconds: Double; public var approvalFingerprint: String?; public var configuration: ActionConfiguration
    public init(id: String = UUID().uuidString, executable: String, arguments: [String] = [], workingDirectory: String? = nil, environment: [String: EnvironmentValue] = [:], singleInstanceKey: String, restartPolicy: RestartPolicy = .never, gracefulStopSeconds: Double = 5, approvalFingerprint: String? = nil, configuration: ActionConfiguration = .init(idempotencyPolicy: .singleInstance)) {
        self.id = id; self.executable = executable; self.arguments = arguments; self.workingDirectory = workingDirectory; self.environment = environment; self.singleInstanceKey = singleInstanceKey; self.restartPolicy = restartPolicy; self.gracefulStopSeconds = gracefulStopSeconds; self.approvalFingerprint = approvalFingerprint; self.configuration = configuration
    }
}
...
extension SceneAction: Codable {
    private enum CodingKeys: String, CodingKey {
        ...
        case singleInstanceKey, restartPolicy, gracefulStopSeconds, durationSeconds, message
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .managedProcess(let v): try common(.managedProcess, v.id, v.configuration); try encodeProcess(v.executable, v.arguments, v.workingDirectory, v.environment, v.approvalFingerprint, to: &c); try c.encode(v.singleInstanceKey, forKey: .singleInstanceKey); try c.encode(v.restartPolicy, forKey: .restartPolicy); try c.encode(v.gracefulStopSeconds, forKey: .gracefulStopSeconds)
